...
export const getOwnerAnalytics = async (ownerId: number) => {
  ...
  const topOwners = allOwnerStats
      ...
      const totalIncome = owner.ownedProperties.reduce(
        (sum, property) =>
          ...
          property.rooms.reduce(
            (roomSum, room) =>
              roomSum + room.bookings.reduce((bookingSum, booking) => bookingSum + Number(booking.ownerIncome), 0),
            0,
          ),
        0,
      )
      const ratings = owner.ownedProperties.flatMap((property) => property.reviews.map((review) => review.rating))
    ...
    .filter((owner) => owner.totalIncome > 1000)
    .sort((left, right) => {
